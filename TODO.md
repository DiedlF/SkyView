...
- **Status:** Derived radar dBZ + satellite HRV ingest, serving endpoints, frontend controls/loop, status freshness, systemd timer files, GitHub deploy wiring, and render-grid georeferencing are implemented and live-verified 2026-06-13. Both products now render to a single ``TARGET_GRID`` (a window a bit wider than the ICON-EU/D2 crop: 40–60°N, −8–26°E at 0.02°) and the serving layer advertises that same grid as the frame bbox for both sources, so the frontend image overlay lines up with the basemap (regression guard: `tests/test_observations_api.py::test_served_bbox_matches_render_grid`). Remaining work: post-deploy visual QA, legal page copy, backfill/refetch tooling for derived-only cache refreshes, and optional numeric-grid path.
