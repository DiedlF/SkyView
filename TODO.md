...
- **MTG-I1 Lightning Imager (LI) overlay (added 2026-06-15):** a fourth observation source (`li`) renders the gridded LI L2 **Accumulated Flashes** product (`EO:EUM:DAT:0686`, satpy `li_l2_nc`, dataset `flash_accumulation`) as a coloured, mostly-transparent yellow→white overlay on its own pane *above* the satellite layers — so it pairs with both the MSG HRV and MTG vis_06 images (MSG/SEVIRI has no lightning sensor, so this is the only lightning option for the HRV layer). New `backend/observations/satellite_li.py`, `render_li_accum_png`, `ingest_li`, router `li` source, frontend toggle/pane/labels (EN "Lightning (MTG-I1)" / DE "Blitze"). **Cheap (bandwidth investigated live 2026-06-15):** one ~2–3 MB single-chunk product per 10-min cycle (~330 MB/day typical), so no chunk subsetting — just dedup-before-download. Live-verified end-to-end against the real Data Store (rendered a sparse flash cluster correctly georeferenced over E Germany/Poland). Needs the LI NRT licence (`EO:EUM:DAT:0686`) accepted on the VPS, same as the other collections.
- **Ops:** satellite downloads need the per-collection **NRT licence** accepted in the EUMETSAT Data Store (documented in `docs/OPS_SECRETS.md`). This now includes the FCI HRFI collection (`EO:EUM:DAT:0665`) for the MTG-I1 vis layer and the LI Accumulated Flashes collection (`EO:EUM:DAT:0686`) for the lightning overlay.
